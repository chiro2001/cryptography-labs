 ...
 * 列混合要的矩阵
 */
int colM[4][4] = {2, 3, 1, 1, 1, 2, 3, 1, 1, 1, 2, 3, 3, 1, 1, 2};
/**
 * 逆列混合的矩阵
 */
int deColM[4][4] = {0xe, 0xb, 0xd, 0x9, 0x9, 0xe, 0xb, 0xd,
 ...
 * 把一个字符转变成整型
 */
int getIntFromChar(char c) { return ((int) c) & 0x000000ff; }
...
void shiftArrayOneStep(int array[4], int isLeft) {
  int t;
  if (isLeft) {
    t = array[0];
    array[0] = array[1];
...
void leftLoop4int(int array[4], int step) {
  for (int i = 0; i < step; i++) shiftArrayOneStep(array, 1);
}
...
void rightLoop4int(int array[4], int step) {
  for (int i = 0; i < step; i++) shiftArrayOneStep(array, 0);
}
...
void shiftRows(int array[4][4]) {
  for (int i = 0; i < 4; i++) leftLoop4int(array[i], i);
}
...
void deShiftRows(int array[4][4]) {
  for (int i = 0; i < 4; i++) rightLoop4int(array[i], i);
}
...
void convertArrayToStr(int array[4][4], char *str) {
  int i, j;
  for (i = 0; i < 4; i++)
    for (j = 0; j < 4; j++) *str++ = (char) array[j][i];
}
...
void aesStrToFile(char *key) {
  ...
  if (test->plain_text == NULL) {
    printf("请输入你的明文，明文字符长度必须为16的倍数\n");
    readPlainText(p, &plen);
  } else {
    printf("你输入的明文为：%s\n", test->plain_text);
    strcpy(p, test->plain_text);
    plen = strlen(test->plain_text);
  }
